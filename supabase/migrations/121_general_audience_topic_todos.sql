-- 121: General-audience topic "Todos" + feed eligibility without forced distribution
-- "Todos" expands who can see a post; ranking still uses recency/quality/follows/sector.
...
CREATE OR REPLACE FUNCTION public.is_general_audience_post(p_post_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.is_general_audience_post(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.is_general_audience_post(UUID) TO authenticated, service_role;

-- Patch personalized feed: modest +10 for general posts + eligibility gate for candidates.
...
BEGIN
  ...
  IF v_feed_mode = 'candidate' THEN
    ...
    posts_pool AS (
        ...
        SELECT
          ...
          (
            ...
            + CASE
                ...
                THEN 15 ELSE 0
              END
            -- General audience ("Todos"): eligible for everyone, modest boost (not spam).
            + CASE WHEN public.is_general_audience_post(p.id) THEN 10 ELSE 0 END
      ...
      WHERE scored.relevance_score >= 12
         OR EXISTS (
           SELECT 1 FROM followed_ids fd
           WHERE fd.target_id = (scored.payload ->> 'author_id')::UUID
         )
         OR public.is_general_audience_post((scored.payload ->> 'id')::UUID)
    ...
    posts_pool AS (
      SELECT
        ...
        (
          ...
          + CASE WHEN public.is_general_audience_post(p.id) THEN 8 ELSE 0 END
        )::INT AS relevance_score,
        p.created_at AS sort_at,
        to_jsonb(p) AS payload
      FROM public.posts p
      WHERE coalesce(p.is_hidden, false) = false
        ...
        AND (
          ...
          OR public.is_general_audience_post(p.id)
        )
