...
USING (bucket_id = 'candidate_avatars');
...
USING (bucket_id = 'candidate_avatars' AND auth.uid() = (storage.foldername(name))[1]::uuid)
WITH CHECK (bucket_id = 'candidate_avatars' AND auth.uid() = (storage.foldername(name))[1]::uuid);

-- Bucket: candidate_cvs
...
ON storage.objects FOR ALL
USING (bucket_id = 'candidate_cvs' AND auth.uid() = (storage.foldername(name))[1]::uuid)
WITH CHECK (bucket_id = 'candidate_cvs' AND auth.uid() = (storage.foldername(name))[1]::uuid);