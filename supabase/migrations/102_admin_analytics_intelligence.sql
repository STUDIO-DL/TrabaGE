-- Admin Analytics & Labor Market Intelligence (Phase 1)
-- Aggregated, admin-only RPCs. No PII in return payloads.
-- Youth/age analytics intentionally omitted until voluntary age fields exist.
...
BEGIN
  ...
  job_titles AS (
    SELECT
      ...
      (
        SELECT count(*)::INT
        FROM filtered_apps a
        JOIN filtered_jobs j2 ON j2.id = a.job_id
        WHERE lower(coalesce(nullif(trim(j2.title), ''), 'Sin título')) = lower(coalesce(nullif(trim(fj.title), ''), 'Sin título'))
          AND a.applied_at >= v_from AND a.applied_at < v_to
      ) AS applications
    FROM filtered_jobs fj
    WHERE fj.created_at >= v_from AND fj.created_at < v_to
    GROUP BY 1
    HAVING count(*) >= 1
    ORDER BY offers DESC, applications DESC
    LIMIT 25
  ),
  sector_activity AS (
    SELECT
      ...
      (
        ...
        FROM filtered_apps a
        JOIN filtered_jobs j2 ON j2.id = a.job_id
        WHERE j2.sector = fj.sector
          AND a.applied_at >= v_from AND a.applied_at < v_to
      ) AS applications,
      (
        SELECT count(*)::INT FROM personal_users pu
        WHERE lower(coalesce(pu.sector, '')) = lower(fj.sector)
      ) AS candidates
    FROM filtered_jobs fj
    WHERE fj.created_at >= v_from AND fj.created_at < v_to
    GROUP BY sector
  ...
  geo_activity AS (
    SELECT
      ...
      (
        SELECT count(*)::INT FROM personal_users pu
        WHERE lower(coalesce(pu.city, '')) = lower(coalesce(fj.city, ''))
      ) AS candidates,
      (
        SELECT count(*)::INT FROM employer_users eu
        WHERE lower(coalesce(eu.city, '')) = lower(coalesce(fj.city, ''))
      ) AS employers,
      (
        SELECT count(*)::INT
        FROM filtered_apps a
        JOIN filtered_jobs j2 ON j2.id = a.job_id
        WHERE lower(coalesce(j2.city, '')) = lower(coalesce(fj.city, ''))
          AND a.applied_at >= v_from AND a.applied_at < v_to
      ) AS applications
    FROM filtered_jobs fj
    WHERE fj.created_at >= v_from AND fj.created_at < v_to
    GROUP BY coalesce(nullif(trim(city), ''), 'Sin ciudad'), lower(coalesce(city, ''))
    ORDER BY offers DESC
    LIMIT 25
  ),
  skills_demand AS (
...
CREATE INDEX IF NOT EXISTS idx_candidate_profiles_sector ON public.candidate_profiles (sector);
