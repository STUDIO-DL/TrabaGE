...
/**
 * Security notifications that we acknowledge without sending (or with a dedicated
 * template) so Auth operations like password update are not blocked by the hook.
 */
function isAckOnlyNotification(actionType: string) {
  return (
...
Deno.serve(async (req) => {
  ...
  // Never fail password/security Auth flows because of an unimplemented notification.
  if (isAckOnlyNotification(actionType)) {
    console.log('[send_auth_email] ack unsupported notification:', actionType);
  ...
  try {
    ...
    // Password-change notification must not block the password update itself.
    // Client also sends a best-effort confirmation via notify_password_changed.
    if (actionType === 'password_changed_notification') {
      console.error(
        '[send_auth_email] password_changed_notification delivery failed; acknowledging hook',
      );
      return hookSuccessResponse();
    }
