...
function mapJobError(error) {
  ...
  if (message.includes('foreign key')) {
    return { ...error, message: 'No se encontró el perfil asociado.' };
  }
...
export const jobsService = {
  ...
  /**
   * Personal-user shared opportunity (simpler payload than company offers).
   * Optionally uploads an image after insert.
   */
  createSharedOpportunity: async ({ userId, title, description, city, contactMethod, imageFile }) => {
    ...
    const insertResult = await supabase.from('jobs').insert(basePayload).select().single();
    if (insertResult.error) {
      return { ...insertResult, error: mapJobError(insertResult.error) };
    }

    let job = insertResult.data;

    if (imageFile && job?.id) {
      const upload = await storageService.uploadJobOpportunityImage(userId, job.id, imageFile);
      if (!upload.error) {
        const imagePath = versionedStoragePath(upload.path, STORAGE_BUCKETS.POST_IMAGES);
  ...
  getCompanyJobs: async (companyId, options = {}) => {
    let query = supabase
      .from('jobs')
      .select(
        'id, title, role, description, city, country, job_type, work_mode, salary, salary_negotiable, status, created_at, company_id, application_deadline, source_type, applications_count:applications(count)',
      )
  ...
  notifyJobPublished: async (job) => {
    ...
    if (job.source_type === JOB_SOURCE.USER) return { error: null };
