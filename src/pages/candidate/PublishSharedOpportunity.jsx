...
export default function PublishSharedOpportunity() {
  ...
  const profileIncomplete =
    !isPreviewMode && !profileLoading && !isSharedOpportunityProfileComplete(profile);
  ...
  const handleSubmit = async (event) => {
    ...
    if (role !== ROLES.PERSONAL) {
      setError('Solo las cuentas personales pueden publicar oportunidades compartidas.');
      return;
    }
  ...
  if (isPreviewMode) {
    return (
      <PageContainer title="Publicar oportunidad" backButton bottomNav={false}>
        <div className="p-md">
          <EmptyState title="Inicia sesión" description={GUEST_MODE_MESSAGE} />
    ...
    return (
      <PageContainer title="Publicar oportunidad" backButton bottomNav={false}>
        <div className="p-md">
          ...
          />
          <Link to="/personal/profile/edit-intro" className="mt-md block">
            <Button fullWidth className="btn-primary-mobile !rounded-btn-primary !py-0">
              Completar perfil
            </Button>
          </Link>
    ...
    >
      ...
      >
        <p className="rounded-radius-md border border-app-border bg-app-surface px-space-base py-space-sm text-caption text-app-muted">
          ...
          Estás publicando una <strong className="font-semibold text-app-text">oportunidad compartida</strong>.
          No es una oferta oficial de empresa.
        </p>

        <p className="text-caption text-app-subtle">
          Los campos marcados con <span className="text-red-600" aria-hidden="true">*</span> son
          obligatorios.
        </p>
        ...
        <Card padding="md" className="space-y-4">
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium text-gray-900">Imagen (opcional)</p>
            {imagePreview ? (
              <div className="space-y-2">
                <img
                  src={imagePreview}
