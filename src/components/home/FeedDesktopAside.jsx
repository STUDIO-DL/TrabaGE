...
export default function FeedDesktopAside() {
  ...
  return (
    <aside className="hidden w-[280px] shrink-0 lg:block" aria-label="Atajos">
      <div className="sticky top-4 space-y-space-base">
        <div className="rounded-radius-md border border-app-border bg-app-card p-space-base shadow-elevation-1">
        ...
        <nav className="overflow-hidden rounded-radius-md border border-app-border bg-app-card shadow-elevation-1">
          <ul className="divide-y divide-app-divider">
            {links.map(({ suffix, label, hint }) => (
              <li key={suffix}>
                <Link
                  to={rolePath(role, suffix)}
                  className="block px-space-base py-space-sm transition-colors hover:bg-app-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-500"
                >
                  <span className="block text-body-small font-medium text-app-text">{label}</span>
