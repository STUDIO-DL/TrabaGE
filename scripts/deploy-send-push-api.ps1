...
try {
  ...
  # JWT verified by function code (user / admin / service role). Gateway verify keeps clients honest.
  $metadata = @{
    name = $functionName
  ...
  $files = @(
    ...
    @{ Path = (Join-Path $sharedDir "onesignal.ts"); Name = "../_shared/onesignal.ts" }
  )
