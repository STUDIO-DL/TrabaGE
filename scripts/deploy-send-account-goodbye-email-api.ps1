...
$functionName = "send_account_goodbye_email"
...
$authEmailDir = Resolve-Path (Join-Path $PSScriptRoot "..\supabase\functions\send_auth_email")
...
try {
  ...
  # verify_jwt=false: after account deletion the user JWT is invalid; auth is the one-time outbox token.
  $metadata = @{
    name = $functionName
  ...
  $files = @(
    ...
    @{ Path = (Join-Path $authEmailDir "templates.ts"); Name = "../send_auth_email/templates.ts" },
