...
const OUT_DIR = path.join(__dirname, 'profile-verify-output');
...
async function createTestUser(admin, testCase) {
  ...
  const { data: profile } = await admin
    ...
    .select('user_id, company_name, logo_path, is_verified')
    .eq('user_id', userId)
    .maybeSingle();

  return { email, userId, role: roleRow?.role, profile };
}
...
async function login(page, email) {
  ...
  const outcome = await Promise.race([
    page.waitForFunction(() => !window.location.pathname.startsWith('/login'), { timeout: 25000 }).then(() => 'ok'),
    page.locator('[role="alert"]').first().waitFor({ timeout: 25000 }).then(() => 'error'),
  ]).catch(() => 'timeout');
  ...
  if (outcome !== 'ok') throw new Error('login timeout');
}
...
async function runCase(page, admin, testCase, checklist, logoFiles) {
  const section = testCase.id;
  let userId = null;

  try {
    const created = await createTestUser(admin, testCase);
    ...
    await login(page, created.email);
    ...
    if (uploadStatus === 'pass') {
      ...
      const dbLogo = (
        await admin.from('company_profiles').select('logo_path').eq('user_id', userId).maybeSingle()
      ).data?.logo_path;
...
async function main() {
  ...
  for (const testCase of CASES) {
    console.log(`\n--- ${testCase.id} ---\n`);
    await runCase(page, admin, testCase, checklist, logoFiles);
  }
